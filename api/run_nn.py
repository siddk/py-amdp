...
def train_model(step=20): 
    ...
    model = NNDual(data['L0'][0], data['L1'][0], data['L2'][0], data['L0'][2], data['L1'][2], 
                   data['L2'][2])
    ...
    model.saver.save(model.session, 'nn_ckpt/nn.ckpt')
    with open('nn_ckpt/vocab.pik', 'w') as f:
        pickle.dump((model.l0_pc, model.l1_pc, model.l2_pc, model.l0_commands, model.l1_commands,
                     model.l2_commands), f)

def load_model():
    with open('nn_ckpt/vocab.pik', 'r') as f:
        l0_pc, l1_pc, l2_pc, l0_commands, l1_commands, l2_commands = pickle.load(f)
    model = NNDual(l0_pc, l1_pc, l2_pc, l0_commands, l1_commands, l2_commands)
    model.saver.restore(model.session, "nn_ckpt/nn.ckpt")
    return model

if __name__ == "__main__":
    if not os.path.exists("nn_ckpt/checkpoint"):
        train_model()
    m = load_model()
    print 'Model Loaded!'
    while True:
